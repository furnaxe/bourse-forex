...
print(custom_fig.renderText('Noesis Forex'))

while True:
    try:
        print(f"Tentative n°{compteur}")
        ...
        print("Niveau le plus élevé des 3 derniers jours: " + str(current_day_high))
        current_day_low = float(min(current_day_data, key=lambda x: float(x['3. low']))['3. low'])
        print("Niveau le moins élevé des 3 derniers jours: " + str(current_day_low ))
        current_day_close = float(top_3days_data['4. close'])
        print("Dernier niveau de clôture: " + str(current_day_close))
        
        # Vérifier si l'écart entre le minimum et le maximum est suffisamment grand
        if current_day_high - current_day_low < 0.01:
            print("L'écart entre le minimum et le maximum est inférieur à 100 points de base. Impossible d'ouvrir une position.")
            time.sleep(180)
            continue
